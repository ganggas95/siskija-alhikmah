...
import { DeleteWithConfirmForm } from "@/components/form/delete-with-confirm-form";
...
export default async function ExpensePage({
  ...
  const { page, skip, take, pageSize } =
    getPaginationState(resolvedSearchParams);
  ...
  const where: Prisma.ExpenseTransactionWhereInput = {
    ...
    ...(statusFilter && statusFilter !== "all"
      ? { status: statusFilter as ExpenseStatus }
      : {}),
    ...(categoryIdFilter && categoryIdFilter !== "all"
      ? { categoryId: categoryIdFilter }
      : {}),
    ...(methodFilter && methodFilter !== "all"
      ? { method: methodFilter as PaymentMethod }
      : {}),
  };
        ...
        <div className="mb-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <div className="flex gap-3">
              ...
              <TableLoadingLink
                href="/kas-keluar"
                className="inline-flex items-center rounded-xl px-3 py-3 text-sm font-medium text-green-800"
              >
        ...
        <div className="space-y-3 md:hidden">
          {transactions.length > 0 ? (
                ...
                <div className="mt-3 flex items-center justify-end gap-2">
                  ...
                  {transaction.status === ExpenseStatus.DRAFT ? (
                    <>
                      ...
                      <DeleteWithConfirmForm
                        action={deleteExpenseAction}
                        id={transaction.id}
                        confirmMessage="Hapus transaksi kas keluar ini?"
                        buttonClassName="rounded-lg border border-red-300 px-3 py-2 text-xs font-medium text-red-600"
                      />
                    </>
                  ) : null}
                </div>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">
              <TableEmptyState
                icon={ArrowUpCircle}
                title="Belum ada transaksi kas keluar"
                    ...
                    {transaction.status === ExpenseStatus.DRAFT ? (
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <DeleteWithConfirmForm
                          ...
                          id={transaction.id}
                          confirmMessage="Hapus transaksi kas keluar ini?"
                          buttonClassName="text-sm font-medium text-red-600"
                        />
                      </div>
                    ) : (
